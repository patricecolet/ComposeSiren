...
void SirenePlugAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (auto sample = 0; sample < buffer.getNumSamples(); sample++) {
        ...
        // TODO : move all mySynth->sx->fun() calls
        //  into synth.cpp, where they belong
        //  Also, an array of Sirene instances would be nice
...
{
    // todo : do not expose Sirene instances here
    mySynth->s1->setnote();
    mySynth->s2->setnote();
    mySynth->s3->setnote();
    mySynth->s4->setnote();
